...
const DashboardInventoryPage = () => {
  ...
  useEffect(() => {
    const carregarProdutos = async () => {
      try {
        const response = await apiRequester.get("/api/v1/produtor-produtos/me", {
          params: {
            status: "STATUS_ATIVO"
          }
        });
  ...
  const handleDelete = (id) => {
    if (window.confirm("Tem certeza que deseja excluir este produto?")) {
      apiRequester.put("/api/v1/produtor-produtos/desativar/" + id);
  ...
  const handleSaveProduct = async (selectedProducts) => {
    ...
    try {
      ...
      const response = await apiRequester.post("/api/v1/produtor-produtos/solicitar-venda", {
      ...
      const uniqueNewProducts = newProducts.filter((np) => !produtos.some((p) => p.id === np.id));
  ...
  if (errorId) {
    return (
      <div className={styles.wrapper}>
        <p style={{ color: "red" }}>Erro ao carregar dados: {errorId}</p>
      </div>
    );
  }
  // Fim da correção
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <InventoryTable produtos={produtos} onEdit={handleEdit} onDelete={handleDelete} />

      {isModalOpen && (
        <AddProductModal onClose={handleCloseModal} onSave={handleSaveProduct} produtoInicial={produtoParaEditar} />
      )}
    </div>
  );
};
